...
def equation_of_time(n):
    b =  math.radians((n - 1) * 360 / 365) # have to import math then use math.radians
    Equation_of_Time = 229.2 * (7.5 * 10 ** -5 + 0.001868 * math.cos(b) - 0.014615 * math.cos(2 * b) - 0.04089 * math.sin(2 * b))
...
def Declination_Angle(n):
    DeclinationAngle = 23.45 * math.sin(2 * pi * (284 + n) / 365)
...
def Sunset_Angle(month, day, latitude):
    ...
    SunsetAngle = math.degrees( math.acos(-math.tan(Lat_rad) * math.tan(delta_rad)))
    return SunsetAngle
...
def Solar_radiation_on_horizontal_GHI(month, day, latitude, H_bar, SolarTime):
    ...
    omega_rad = math.radians(omega_deg)             # the sunset angle in radians
    Hour_angle_rad = math.radians(Hour_angle(SolarTime))   # hour angle in radians
    a = 0.409 + 0.5016 * math.sin(omega_rad - pi/3)      # constant in formula
    b = 0.6609 - 0.4767 * math.sin(omega_rad - pi/3)     # constant in formula
    if Hour_angle_rad < omega_rad or Hour_angle_rad > omega_rad:
        r_h = 0
    else: 
        r_h = pi / 24 * (a + b * math.cos(Hour_angle_rad)) * (math.cos(Hour_angle_rad) - math.cos(omega_rad))/(math.sin(omega_rad) - omega_rad * math.cos(omega_rad))
    I_bar = r_h * H_bar
    return I_bar
...
def Solar_alitude_angle(latitude, DeclinationAngle, SolarTime):
    lat_rad = math.radians(latitude)
    delta_rad = math.radians(DeclinationAngle)
    Hour_angle_rad = math.radians(SolarTime)
    SolarAltitudeAngle = np.rad2deg(math.asin(math.cos(lat_rad) * math.cos(Hour_angle_rad) + math.sin(lat_rad) * math.sin(delta_rad)))
    return SolarAltitudeAngle

# Solar Azimuth Angle
def Solar_azimuth_angle(latitude, DeclinationAngle, SolarTime, SolarAltitudeAngle):
    Solar_alitude_angle_rad = math.radians(SolarAltitudeAngle)
    lat_rad = math.radians(latitude)
    delta_rad = math.radians(DeclinationAngle)
    if SolarTime > 12:
        SolarAzimuthAngle = np.rad2deg(math.acos((math.sin(Solar_alitude_angle_rad) * math.sin(lat_rad) - math.sin(delta_rad)) / (math.cos(Solar_alitude_angle_rad) * math.cos(lat_rad))))
    else:
        SolarAzimuthAngle = np.rad2deg(math.acos((math.sin(Solar_alitude_angle_rad) * math.sin(lat_rad) - math.sin(delta_rad)) / (math.cos(Solar_alitude_angle_rad) * math.cos(lat_rad))))* -1
...
def Solar_incidence_angle_south(SurfaceAzimuthAngle_south, TiltAngle_deg_south, SolarAltitudeAngle_deg, SolarAzimuthAngle, n):
    SolarAltitudeAngle_rad = math.radians(SolarAltitudeAngle_deg)
    SurfaceSolarAzimuth_south_rad = math.radians(SolarAzimuthAngle - SurfaceAzimuthAngle_south)
    TiltAngle_south_rad = math.radians(TiltAngle_deg_south)
    SolarIncidenceAngle_south = np.rad2deg(math.acos(SolarAltitudeAngle_rad * math.cos(SurfaceSolarAzimuth_south_rad) * math.sin(TiltAngle_south_rad) + math.sin(SolarAltitudeAngle_rad) * math.cos(TiltAngle_south_rad)))
...
def Solar_incidence_angle_north(SurfaceAzimuthAngle_north, TiltAngle_deg_north, SolarAltitudeAngle_deg, SolarAzimuthAngle, n):
    SolarAltitudeAngle_rad = math.radians(SolarAltitudeAngle_deg)
    SurfaceSolarAzimuth_north_rad = math.radians(SolarAzimuthAngle - SurfaceAzimuthAngle_north)
    TiltAngle_north_rad = math.radians(TiltAngle_deg_north)
    SolarIncidenceAngle_north = np.rad2deg(math.acos(SolarAltitudeAngle_rad * math.cos(SurfaceSolarAzimuth_north_rad) * math.sin(TiltAngle_north_rad) + math.sin(SolarAltitudeAngle_rad) * math.cos(TiltAngle_north_rad)))
...
def Solar_incidence_angle_east(SurfaceAzimuthAngle_east, TiltAngle_deg_east, SolarAltitudeAngle_deg, SolarAzimuthAngle, n):
    SolarAltitudeAngle_rad = math.radians(SolarAltitudeAngle_deg)
    SurfaceSolarAzimuth_east_rad = math.radians(SolarAzimuthAngle - SurfaceAzimuthAngle_east)
    TiltAngle_east_rad = math.radians(TiltAngle_deg_east)
    SolarIncidenceAngle_east = np.rad2deg(math.acos(SolarAltitudeAngle_rad * math.cos(SurfaceSolarAzimuth_east_rad) * math.sin(TiltAngle_east_rad) + math.sin(SolarAltitudeAngle_rad) * math.cos(TiltAngle_east_rad)))
...
def Solar_incidence_angle_west(SurfaceAzimuthAngle_west, TiltAngle_deg_west, SolarAltitudeAngle_deg, SolarAzimuthAngle, n):
    SolarAltitudeAngle_rad = math.radians(SolarAltitudeAngle_deg)
    SurfaceSolarAzimuth_west_rad = math.radians(SolarAzimuthAngle - SurfaceAzimuthAngle_west)
    TiltAngle_west_rad = math.radians(TiltAngle_deg_west)
    SolarIncidenceAngle_west = np.rad2deg(math.acos(SolarAltitudeAngle_rad * math.cos(SurfaceSolarAzimuth_west_rad) * math.sin(TiltAngle_west_rad) + math.sin(SolarAltitudeAngle_rad) * math.cos(TiltAngle_west_rad)))
...
def H_bar_0(month, day, latitude, n):
    ...
    Hbar_0 = 86400 * 2.77778 * (10 ** -7) * Gsc / pi * (1 + 0.033 * math.cos(2 * pi * n / 365)) * (math.cos(latitude_rad) * math.cos(Declination_Angle_rad) * math.sin(omega_rad) + omega_rad * math.sin(latitude_rad) * math.sin(Declination_Angle_rad))
    return Hbar_0
...
def H_bar_d_south(month, day, latitude, H_bar, TiltAngle_deg_south, n):
   ...
   # Declination_Angle_rad = math.radians(Declination_Angle_deg)
    ...
    k_t = H_bar / Hbar_0
    if omega_deg < 81.4 :
        H_bar_d_south = max(0, (1.391 - 3.56 * k_t + 4.189 * k_t ** 2 - 2.137 * k_t ** 3) * H_bar * (1 + math.cos(TiltAngle_south_rad))/2 )
    else:
        H_bar_d_south = max(0, (1.311 - 3.022 * k_t + 3.427 * k_t ** 2 - 1.821 * k_t ** 3) * H_bar * ( 1 + math.cos(TiltAngle_south_rad))/2 )
...
def H_bar_d_north(month, day, latitude, H_bar, TiltAngle_deg_north, n):
   ...
   # Declination_Angle_rad = math.radians(Declination_Angle_deg)
    ...
    k_t = H_bar / Hbar_0
    if omega_deg < 81.4 :
        H_bar_d_north = max(0, (1.391 - 3.56 * k_t + 4.189 * k_t ** 2 - 2.137 * k_t ** 3) * H_bar * (1 + math.cos(TiltAngle_north_rad))/2 )
    else:
        H_bar_d_north = max(0, (1.311 - 3.022 * k_t + 3.427 * k_t ** 2 - 1.821 * k_t ** 3) * H_bar * ( 1 + math.cos(TiltAngle_north_rad))/2 )
...
def H_bar_d_east(month, day, latitude, H_bar, TiltAngle_deg_east, n):
   ...
   # Declination_Angle_rad = math.radians(Declination_Angle_deg)
    ...
    k_t = H_bar / Hbar_0
    if omega_deg < 81.4 :
        H_bar_d_east = max(0, (1.391 - 3.56 * k_t + 4.189 * k_t ** 2 - 2.137 * k_t ** 3) * H_bar * (1 + math.cos(TiltAngle_east_rad))/2 )
    else:
        H_bar_d_east = max(0, (1.311 - 3.022 * k_t + 3.427 * k_t ** 2 - 1.821 * k_t ** 3) * H_bar * ( 1 + math.cos(TiltAngle_east_rad))/2 )
    return H_bar_d_east
...
def H_bar_d_west(month, day, latitude, H_bar, TiltAngle_deg_west, n):
   ...
   # Declination_Angle_rad = math.radians(Declination_Angle_deg)
    ...
    k_t = H_bar / Hbar_0
    if omega_deg < 81.4 :
        H_bar_d_west = max(0, (1.391 - 3.56 * k_t + 4.189 * k_t ** 2 - 2.137 * k_t ** 3) * H_bar * (1 + math.cos(TiltAngle_west_rad))/2 )
    else:
        H_bar_d_west = max(0, (1.311 - 3.022 * k_t + 3.427 * k_t ** 2 - 1.821 * k_t ** 3) * H_bar * ( 1 + math.cos(TiltAngle_west_rad))/2 )
    return H_bar_d_west
...
def Beam_solar_radiation_south(month, day, latitude, H_bar, SolarTime, SolarAltitudeAngle, SolarIncidenceAngle_south,TiltAngle_deg_south,n):
    ...
    Solar_Incidence_Angle_south_rad = math.radians(SolarIncidenceAngle_south)
    SolarZeinthAngle_rad = math.radians(90 - SolarAltitudeAngle)
    
    r_b = math.cos(Solar_Incidence_Angle_south_rad)/math.cos(SolarZeinthAngle_rad)
...
def Beam_solar_radiation_north(month, day, latitude, H_bar, SolarTime, SolarAltitudeAngle, SolarIncidenceAngle_north,TiltAngle_deg_north,n):
    ...
    omega_rad = math.radians(omega_deg)
    HourAngle_rad = math.radians(Hour_angle(SolarTime))
    Solar_Incidence_Angle_north_rad = math.radians(SolarIncidenceAngle_north)
    SolarZeinthAngle_rad = math.radians(90 - SolarAltitudeAngle)
    
    r_b = math.cos(Solar_Incidence_Angle_north_rad)/math.cos(SolarZeinthAngle_rad)
    if HourAngle_rad < - omega_rad or HourAngle_rad > omega_rad:
        r_d = 0
    else:
        r_d = pi/24 * (math.cos(HourAngle_rad) - math.cos(omega_rad)) / (math.sin(omega_rad) - omega_rad * math.cos(omega_rad))
...
def Beam_solar_radiation_east(month, day, latitude, H_bar, SolarTime, SolarAltitudeAngle, SolarIncidenceAngle_east,TiltAngle_deg_east,n):
    ...
    omega_rad = math.radians(omega_deg)
    HourAngle_rad = math.radians(Hour_angle(SolarTime))
    Solar_Incidence_Angle_east_rad = math.radians(SolarIncidenceAngle_east)
    SolarZeinthAngle_rad = math.radians(90 - SolarAltitudeAngle)
    
    r_b = math.cos(Solar_Incidence_Angle_east_rad)/math.cos(SolarZeinthAngle_rad)
    if HourAngle_rad < - omega_rad or HourAngle_rad > omega_rad:
        r_d = 0
    else:
        r_d = pi/24 * (math.cos(HourAngle_rad) - math.cos(omega_rad)) / (math.sin(omega_rad) - omega_rad * math.cos(omega_rad))
...
def Diffuse_solar_radiation_south(month, day, latitude, H_bar, SolarTime, TiltAngle_deg_south,n):
    ...
    if HourAngle_rad < - omega_rad or HourAngle_rad > omega_rad:
        ...
        pi / 24 * (math.cos(HourAngle_rad) - math.cos(omega_rad)) / (math.sin(omega_rad) - omega_rad * (math.cos(omega_rad)))
    H_bar_d_0 = H_bar_d_south(month, day, latitude, H_bar, TiltAngle_deg_south, n)
    I_bar_d_horizontal = r_d * H_bar_d_0
    I_bar_d_south = I_bar_d_horizontal * (1 + math.cos(TiltAngle_south_rad)) / 2
...
def Diffuse_solar_radiation_north(month, day, latitude, H_bar, SolarTime, TiltAngle_deg_north,n):
    ...
    I_bar_d_horizontal = r_d * H_bar_d_0
    I_bar_d_north = I_bar_d_horizontal * (1 + math.cos(TiltAngle_north_rad)) / 2
...
def Diffuse_solar_radiation_east(month, day, latitude, H_bar, SolarTime, TiltAngle_deg_east,n):
    ...
    I_bar_d_horizontal = r_d * H_bar_d_0
    I_bar_d_east = I_bar_d_horizontal * (1 + math.cos(TiltAngle_east_rad)) / 2
    return I_bar_d_east
...
def Diffuse_solar_radiation_west(month, day, latitude, H_bar, SolarTime, TiltAngle_deg_west,n):
    ...
    I_bar_d_horizontal = r_d * H_bar_d_0
    I_bar_d_west = I_bar_d_horizontal * (1 + math.cos(TiltAngle_west_rad)) / 2
    return I_bar_d_west
...
def Ground_reflective_south(month, day, latitude, H_bar, TiltAngle_deg_south, SolarTime, T_air):
    ...
    TiltAngle_rad_south = math.radians(TiltAngle_deg_south)
    I_bar_horizontal = Solar_radiation_on_horizontal_GHI(month, day, latitude, H_bar, SolarTime)
    if T_air > 0 :
        rho_g = 0.2
    elif T_air < -5 :
        ...
        rho_g = -0.1 * T_air + 0.2
    I_bar_g_south = I_bar_horizontal * rho_g * ( 1 - math.cos(TiltAngle_rad_south)) / 2
...
def Ground_reflective_north(month, day, latitude, H_bar, TiltAngle_deg_north, SolarTime, T_air):
    ...
    #omega_rad = math.radians(omega_deg)
    #HourAngle = Hour_angle(SolarTime)
    TiltAngle_rad_north = math.radians(TiltAngle_deg_north)
    I_bar_horizontal = Solar_radiation_on_horizontal_GHI(month, day, latitude, H_bar, SolarTime)
    if T_air > 0 :
        rho_g = 0.2
    elif T_air < -5 :
        ...
        rho_g = -0.1 * T_air + 0.2
    I_bar_g_north = I_bar_horizontal * rho_g * ( 1 - math.cos(TiltAngle_rad_north)) / 2
...
def Ground_reflective_east(month, day, latitude, H_bar, TiltAngle_deg_east, SolarTime, T_air):
    ...
    #omega_rad = math.radians(omega_deg)
    #HourAngle = Hour_angle(SolarTime)
    TiltAngle_rad_east = math.radians(TiltAngle_deg_east)
    I_bar_horizontal = Solar_radiation_on_horizontal_GHI(month, day, latitude, H_bar, SolarTime)
    if T_air > 0 :
        rho_g = 0.2
    elif T_air < -5 :
        ...
        rho_g = -0.1 * T_air + 0.2
    I_bar_g_east = I_bar_horizontal * rho_g * ( 1 - math.cos(TiltAngle_rad_east)) / 2
    return I_bar_g_east
...
def Ground_reflective_west(month, day, latitude, H_bar, TiltAngle_deg_west, SolarTime, T_air):
    ...
    #omega_rad = math.radians(omega_deg)
    #HourAngle = Hour_angle(SolarTime)
    TiltAngle_rad_west = math.radians(TiltAngle_deg_west)
    I_bar_horizontal = Solar_radiation_on_horizontal_GHI(month, day, latitude, H_bar, SolarTime)
    if T_air > 0 :
        rho_g = 0.2
    elif T_air < -5 :
        ...
        rho_g = -0.1 * T_air + 0.2
    I_bar_g_west = I_bar_horizontal * rho_g * ( 1 - math.cos(TiltAngle_rad_west)) / 2
    return I_bar_g_west
